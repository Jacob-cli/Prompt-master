...
// ── MODEL LOGOS ───────────────────────────────────────────────────────────────
const LOGO_URLS = {
  zimage_turbo: "https://avatars.githubusercontent.com/u/183588837?s=32&v=4",
  zimage_base:  "https://avatars.githubusercontent.com/u/183588837?s=32&v=4",
  nano_banana:  "https://www.gstatic.com/lamda/images/gemini_sparkle_v002_d4735304ff6292a690345.svg",
  wan22:        "https://avatars.githubusercontent.com/u/165069895?s=32&v=4",
  qwen:         "https://avatars.githubusercontent.com/u/134309484?s=32&v=4",
};
const LOGO_FB = {
...
function ModelLogo({ modelId, size=16 }) {
  ...
  const fb = LOGO_FB[modelId] || { letter:"?", color:C.muted };
  if (!LOGO_URLS[modelId] || failed) {
    return (
      <span style={{ display:"inline-flex", width:size, height:size, borderRadius:3, background:`${fb.color}22`, alignItems:"center", justifyContent:"center", ...M, fontWeight:700, fontSize:Math.round(size*0.6), color:fb.color, flexShrink:0 }}>
        {fb.letter}
      </span>
    );
  }
  return (
    <img src={LOGO_URLS[modelId]} alt="" onError={() => setFailed(true)}
...
export default function Home() {
  ...
  // ── CONFIG BAR ───────────────────────────────────────────────────────────────
  // Row 1: Provider | API Key | Eye toggle | Analysis Model
  // Row 2: Mode buttons | Optimize For pills
  const configBar = (
    <div style={{ borderBottom:`1px solid ${C.border}`, background:C.surface }}>
      ...
      {/* Row 2 */}
      <div style={{ padding:"0 16px 10px", display:"flex", gap:10, flexWrap:"wrap", alignItems:"flex-end" }}>
              ...
              <div key={p.id} style={{ background:C.card,border:`1px solid ${C.border}`,borderRadius:12,display:"flex",gap:12,padding:14,marginBottom:10 }}>
                ...
                <div style={{ flex:1,minWidth:0 }}>
                  <div style={{ display:"flex",gap:6,marginBottom:8,flexWrap:"wrap",alignItems:"center" }}>
                    {p.modelId&&<ModelLogo modelId={p.modelId} size={14}/>}
