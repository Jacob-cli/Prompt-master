...
const C = {
  ...
  border: "#1c1c1c",
  accent: "#b8ff47",
  accentDim: "rgba(184,255,71,0.08)",
  text: "#e0e0e0",
  muted: "#555",
  sub: "#888",
};
...
// ── LOCAL MODEL MARKS ─────────────────────────────────────────────────────────
// Using local marks avoids broken/fake fetched logos and keeps rendering stable.
// Nano Banana uses banana branding. Z-Image / Qwen / Wan keep the same visual family.
function AlibabaFamilyMark({ color = "#d8d8d8", size = 16 }) {
  const r = Math.max(2, Math.round(size * 0.16));
  const g = Math.max(1, Math.round(size * 0.08));
  const cell = Math.max(4, Math.round(size * 0.28));
  const total = cell * 2 + g;
  const pad = Math.max(1, Math.round((size - total) / 2));
  ...
  return (
    ...
    >
      <svg
        ...
        style={{ display: "block" }}
      >
        <rect x={pad} y={pad} width={cell} height={cell} rx={r} fill={color} opacity="0.95" />
        <rect x={pad + cell + g} y={pad} width={cell} height={cell} rx={r} fill={color} opacity="0.78" />
        <rect x={pad} y={pad + cell + g} width={cell} height={cell} rx={r} fill={color} opacity="0.78" />
        <rect
          x={pad + cell + g}
          y={pad + cell + g}
          width={cell}
...
function ModelLogo({ modelId, size = 16 }) {
  if (modelId === "nano_banana") return <BananaMark size={size} />;

  if (modelId === "zimage_turbo") return <AlibabaFamilyMark color="#e9d2ff" size={size} />;
  if (modelId === "zimage_base") return <AlibabaFamilyMark color="#d7b4ff" size={size} />;
  if (modelId === "qwen") return <AlibabaFamilyMark color="#f2c6ff" size={size} />;
  if (modelId === "wan22") return <AlibabaFamilyMark color="#ece69a" size={size} />;
...
const SearchIco = ({ color = "#000", size = 15 }) => (
...
const CopyIco = ({ color = "#000", size = 12 }) => (
  <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
...
const CheckIco = ({ color = C.accent, size = 12 }) => (
  <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth="3" strokeLinecap="round" strokeLinejoin="round">
    <polyline points="20 6 9 17 4 12" />
  </svg>
);

const XIco = ({ color = "#555", size = 13 }) => (
  <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
...
const ChevD = ({ color = C.muted, size = 11 }) => (
  <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
    <polyline points="6 9 12 15 18 9" />
  </svg>
);

const ChevR = ({ color = C.muted, size = 11 }) => (
  <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
  ...
  return (
    <button
      ...
      style={{
        padding: "0 10px",
        borderRadius: 7,
        background: visible ? C.accentDim : "#0d0d0d",
  ...
  return (
    <button
      ...
      style={{
        ...
        borderRadius: 7,
        background: done ? C.accentDim : C.accent,
        color: done ? C.accent : "#000",
    ...
    >
      {done ? (
        <>
          <CheckIco color={C.accent} /> COPIED
        </>
      ) : (
        <>
          <CopyIco color={small ? C.accent : "#000"} /> COPY
        </>
      )}
    </button>
  );
...
function SaveBtn({ onSave }) {
  ...
  return (
    ...
    >
      {done ? (
        <>
          <CheckIco color={C.accent} size={10} /> SAVED
        </>
      ) : (
        "SAVE"
      )}
  ...
  return (
    <button
      ...
      style={{
        ...
        textAlign: "left",
        border: `1px solid ${active ? C.accent : C.border}`,
        background: active ? C.accentDim : "#0d0d0d",
        cursor: "pointer",
        width: "100%",
        transition: "all 0.12s",
      }}
  ...
  return (
    <button
      ...
      style={{
        padding: "6px 11px",
        borderRadius: 999,
        border: `1px solid ${active ? model.color : C.border}`,
        background: active ? `${model.color}15` : "transparent",
        color: active ? model.color : C.muted,
        ...M,
        fontSize: 11,
        cursor: "pointer",
        transition: "all 0.12s",
  ...
  return (
    <div>
      <div
        ...
        onDrop={drop}
        style={{
          border: `2px dashed ${drag ? C.accent : imageData ? "#2a3800" : "#222"}`,
          borderRadius: 12,
          background: drag ? C.accentDim : imageData ? "#0a1200" : "#0d0d0d",
          cursor: "pointer",
          transition: "all 0.15s",
          overflow: "hidden",
          minHeight: imageData ? "auto" : 150,
          display: "flex",
          alignItems: "center",
          justifyContent: "center",
          position: "relative",
...
export default function Home() {
  ...
  useEffect(() => {
    try {
      ...
      const mm = AI_MODELS[prov];
      if (mm) setAiModel(mm[0].id);
    } catch {}
  }, []);

  const persistKeys = (u) => {
    setApiKeys(u);
    localStorage.setItem("skye_keys", JSON.stringify(u));
  };

  const persistLora = (l) => {
    setLoraWord(l);
    localStorage.setItem("skye_lora", l);
  };

  const persistProvider = (p) => {
    setProvider(p);
    localStorage.setItem("skye_provider", p);
    const mm = AI_MODELS[p];
    if (mm) setAiModel(mm[0].id);
  };
  ...
  const runSingle = async () => {
    ...
    try {
      ...
      setOutputs((prev) =>
        prev.map((o) => (o.id === idx ? { ...o, loading: false, prompt, analysis } : o))
      );
    } catch (err) {
  ...
  const saveOutput = (entry, thumbUrl) => {
    ...
    const u = [e, ...saved];
    setSaved(u);
    localStorage.setItem("skye_saved", JSON.stringify(u));
  };
  ...
  const saveSaved = (item) => {
    const e = {
      id: Date.now(),
      model: MODELS.find((m) => m.id === targetModel)?.label || targetModel,
      modelId: targetModel,
      mode,
      prompt: item.prompt,
      thumb: item.img?.dataUrl,
      provider,
    };
  ...
  // ── CONFIG BAR ─────────────────────────────────────────────────────────────
  // Fully block-stacked layout so it behaves properly in portrait + landscape.
  const configBar = (
      ...
      <div className="config-shell">
        ...
        <div className="config-block">
          ...
          <div style={{ display: "flex", gap: 6, width: "100%", alignItems: "stretch" }}>
            <input
              ...
              value={currentKey}
              onChange={(e) => persistKeys({ ...apiKeys, [provider]: e.target.value })}
        ...
        <div className="config-block">
          ...
          <div className="mode-stack">
            <ModeBtn
              label="🎯 Exact Clone"
              desc="full replication"
              active={mode === "exact"}
              onClick={() => setMode("exact")}
            />
            <ModeBtn
              label="✨ LoRA Swap"
              desc={`${loraWord} replaces subject`}
              active={mode === "lora"}
    ...
    >
      ...
      >
        <div className="topbar-shell">
          <div>
            <div style={{ ...M, fontSize: 13, fontWeight: 700, color: C.accent, letterSpacing: "0.1em" }}>
              SKYE PROMPT ENGINE
            </div>
          ...
          <div className="work-grid">
            ...
            <div style={{ minWidth: 0 }}>
              ...
              {mode === "lora" && (
                <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 12, marginBottom: 12 }}>
                  ...
                  <div style={{ padding: 12 }}>
                    ...
                    >
                      <strong style={{ color: C.accent }}>{loraWord}</strong> replaces subject only
                      <br />
                      Scene, lighting, outfit, pose preserved
                      <br />
                      Drift protection: ON
                    </div>
                  </div>
              ...
              <button
                ...
                style={{
                  width: "100%",
                  padding: 13,
                  background: !singleImg || singleLoading ? "#1a1a1a" : C.accent,
                  color: !singleImg || singleLoading ? C.muted : "#000",
                  border: "none",
                  borderRadius: 10,
                  ...M,
                  fontSize: 13,
            ...
            <div style={{ minWidth: 0 }}>
              {outputs.length > 0 && (
                ...
                >
                  <span style={{ ...M, fontSize: 10, color: C.muted }}>
              ...
              {outputs.length === 0 && !singleLoading && (
                <div style={{ textAlign: "center", padding: "80px 20px", color: C.muted }}>
                  ...
                  <div style={{ ...M, fontSize: 10, color: "#252525", marginTop: 8, lineHeight: 1.9 }}>
                    Each click adds a new output box
                    <br />
                    Switch models between runs to compare
                    <br />
      ...
      {tab === "batch" && (
        <div className="main-shell">
          <div className="work-grid batch-grid">
            <div style={{ minWidth: 0 }}>
              <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 12, marginBottom: 12 }}>
                <div
                  style={{
                  ...
                  >
                    <div>
                      <svg width="48" height="48" viewBox="0 0 48 48" fill="none" style={{ display: "block", margin: "0 auto 8px" }}>
                        ...
                        <polyline
                          points="16,25 22,19 27,24 33,17 44,25"
                          stroke={C.accent}
                          strokeWidth="1.5"
                          strokeLinecap="round"
                          ...
                          >
                            ...
                            {res && (
                              <div style={{ position: "absolute", bottom: 3, right: 3 }}>
                                {res.error ? <XIco color="#ff5555" size={11} /> : <CheckIco color={C.accent} size={11} />}
                              </div>
                            )}
                          </div>
                        );
              ...
              <button
                ...
                style={{
                  width: "100%",
                  padding: 13,
                  background: batchImgs.length === 0 || batchLoading ? "#1a1a1a" : C.accent,
                  color: batchImgs.length === 0 || batchLoading ? C.muted : "#000",
                  border: "none",
                  borderRadius: 10,
                  ...M,
                  fontSize: 13,
              ...
              {!batchLoading && batchStatus && (
                <div style={{ marginTop: 8, ...M, fontSize: 10, color: C.accent, textAlign: "center" }}>{batchStatus}</div>
              )}
            </div>
            ...
            <div style={{ minWidth: 0 }}>
              ...
              {batchResults.map((item) => (
                <div key={item.index} style={{ marginBottom: 20, minWidth: 0 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 8, minWidth: 0 }}>
                    <span
                      style={{
                        ...
                        fontSize: 11,
                        color: "#000",
                        background: item.error ? "#ff5555" : C.accent,
                        padding: "3px 10px",
                        borderRadius: 5,
                        fontWeight: 700,
                        flexShrink: 0,
              ...
              <button
                onClick={() => {
                  if (confirm("Clear all?")) {
                    setSaved([]);
                    localStorage.removeItem("skye_saved");
                  }
                }}
                style={{
                  ...M,
                ...
                >
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ display: "flex", gap: 6, marginBottom: 8, flexWrap: "wrap", alignItems: "center" }}>
                      {/* logo intentionally removed in saved cards */}
                      <span
                        style={{
                          ...M,
                          fontSize: 9,
                      ...
                      <span
                        style={{
                          ...M,
                          fontSize: 9,
                          color: p.mode === "lora" ? C.accent : "#47b8ff",
                          border: "1px solid currentColor",
                          padding: "1px 6px",
                          borderRadius: 3,
                        }}
                      ...
                      <button
                        onClick={() => {
                          const u = saved.filter((x) => x.id !== p.id);
                          setSaved(u);
                          localStorage.setItem("skye_saved", JSON.stringify(u));
                        }}
                        style={{
        ...
        <div style={{ maxWidth: 620, margin: "0 auto", padding: 20 }}>
                ...
                <div key={p.id} style={{ marginBottom: 14 }}>
                  ...
                  <div style={{ display: "flex", gap: 6 }}>
                    <input
                      type={keyVisible ? "text" : "password"}
                      value={apiKeys[p.id] || ""}
                      onChange={(e) => persistKeys({ ...apiKeys, [p.id]: e.target.value })}
          ...
          <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 12 }}>
            ...
            <div style={{ padding: 14, ...M, fontSize: 11, color: C.sub, lineHeight: 1.9 }}>
              <strong style={{ color: "#b8ff47" }}>Anthropic Claude</strong> — Best accuracy. console.anthropic.com
              <br />
              <strong style={{ color: "#47b8ff" }}>OpenAI GPT-4o</strong> — Strong vision. platform.openai.com
              <br />
      ...
      <style>{`
        ...
        @media (max-width: 520px) {
          .work-grid,
          .batch-grid {
            grid-template-columns: minmax(160px, 41%) minmax(0, 1fr);
          }
        }
      `}</style>
    </div>
