...
export const MODELS = [
  { id: "zimage_turbo", label: "Z-Image Turbo", color: "#b8ff47", settings: "guidance_scale: 0.0 · steps: 9 · 1024×1024 · NO negative prompt" },
...
POSE AND GESTURE — analyze each component separately:
...
ACCESSORIES: every piece of jewelry — type, material, size, exact placement on body.

ENVIRONMENT — catalog every element:
...
LIGHTING — this is the most critical layer:
...
EXACT mode: Include all subject details — skin tone, skin finish as lighting interaction, makeup details (lip color/finish, lash style, brow character, highlight placement on face).
...
Z-IMAGE TURBO RULES:
- guidance_scale: 0.0 — NO negative prompts (ignored at inference)
- All constraints POSITIVE only. Instead of neg "blurry" write pos "tack-sharp focus"
- 100–220 words optimal. Natural sentence style, not tag-stuffed.
...
OUTPUT: A single optimized prompt paragraph. No analysis text. No bullet points. No JSON. No headers. No explanation. Just the prompt — ready to paste into ComfyUI.`;
...
Preserve everything else exactly from the reference: scene, lighting, pose, outfit, accessories, environment, color grade.`
    : `MODE: Exact Replication — no LoRA
...
${modelId === "zimage_turbo" ? "guidance_scale=0.0 · no negative prompt · all positive · 100–220 words · natural sentences." : ""}
...
Analyze the reference image now. Apply your full forensic visual analysis internally first, then output the optimized prompt. Output the prompt only — nothing else.`;
}
